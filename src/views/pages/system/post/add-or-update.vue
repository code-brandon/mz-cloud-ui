    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
      ...
      <el-col :sm="24" :md="12">
        <el-form-item label="显示顺序" prop="postSort">
          <el-input-number v-model="dataForm.postSort" size="medium" :min="0" label="显示顺序"></el-input-number>
...
<script>
import { getPostInfo,updatePost,savePost } from '@/api/system/post'
export default {
  // 组件名称
  ...
  methods: {
    init(postId) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.postId) {
          getPostInfo(this.dataForm.postId).then(({ data: res }) => {
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          if (!this.dataForm.postId) {
            savePost({ ...this.dataForm }).then(({ data: res }) => {
              if (res.code === this.$OkCode) {
                this.visible = false
            ...
            updatePost({ ...this.dataForm }).then(({ data: res }) => {
              if (res.code === this.$OkCode) {
                this.visible = false
                this.$emit('refreshDataList')
              }
