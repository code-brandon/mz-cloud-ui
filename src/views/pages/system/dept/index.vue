...
<template>
  <div>
    <el-form :inline="true" size="small" :model="param.data" ref="formInline" class="demo-form-inline"
      label-width="68px">
      <el-form-item label="部门名称" prop="deptName">
        <el-input v-model="param.data.deptName" placeholder="请输入菜单名称"></el-input>
      </el-form-item>
      ...
      <el-form-item>
        <CommonSearchReset @reset="onReset" @search="getDeptListTree"></CommonSearchReset>
      </el-form-item>
    </el-form>
    <CommonControlCard @refresh="getDeptListTree">
    ...
    <el-table v-if="refreshTable" :data="tableData" style="width: 100%;margin-bottom: 20px;" row-key="deptId"
      ...
      <el-table-column fixed="right" label="操作" width="180">
        <template v-slot="scope">
          <el-button type="text" icon="el-icon-edit" size="small"
            @click="addOrUpdateHandle({ deptId: scope.row.deptId, type: 1 })">编辑</el-button>
          <el-button type="text" icon="el-icon-plus" size="small"
            @click="addOrUpdateHandle({ deptId: scope.row.deptId, type: 2 })">添加</el-button>
          <el-button type="text" icon="el-icon-delete" size="small" @click="deleteDept(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    getDeptList() {
      getDeptList(this.param).then(({ data: res }) => {
    ...
    getDeptListTree() {
      getDeptListTree(this.param).then(({ data: res }) => {
        this.tableData = res.data;
      }).catch(error => {
        console.error(error)
      })
    ...
    deleteDept(val) {
      ...
      this.$confirm(`确定对[${val.deptName}]进行删除操作?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteDept([val.deptId]).then(({ data: res }) => {
        ...
        this.$message.info(`取消删除[${val.deptName}]`);
      })
    },
    onReset() {
      this.$nextTick(() => {
