  ...
  <div>
    <el-form :inline="true" size="small" :model="param.data" ref="formInline" class="demo-form-inline" label-width="68px">
      <el-form-item label="字典名称" prop="dictName">
        <el-input v-model="param.data.dictName" placeholder="字典名称"></el-input>
      </el-form-item>
      ...
      <el-form-item>
        <CommonSearchReset @reset="onReset" @search="getDictTypePage"></CommonSearchReset>
      </el-form-item>  
    </el-form>

    <CommonControlCard @refresh="getDictTypePage">
      <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="addOrUpdateHandle()">
        新增
      </el-button>
      <el-button type="danger" plain icon="el-icon-delete" size="mini" @click="deleteDictType()">
        删除
      </el-button>
    ...
    <el-table :data="tableData.list" stripe style="width: 100%;" @selection-change="selectionChangeHandle">
      <el-table-column type="selection" header-align="center" align="center" width="50">
      </el-table-column>
      <el-table-column prop="dictId" header-align="center" align="center" label="字典主键">
      </el-table-column>
        ...
        <template v-slot="scope">
          <el-tooltip :content="'Switch value: ' + scope.row.status" placement="top">
            <el-switch v-model="scope.row.status" active-color="#13ce66" inactive-color="#ff4949" active-value="0"
              inactive-value="1" @change="updateDictTypeStatus({sysId:scope.row.dictId,status:scope.row.status})">
            </el-switch>
          </el-tooltip>
        </template>
      </el-table-column>
    ...
    <CommonPagination :currPage.sync="tableData.currPage" :pageSize.sync="tableData.pageSize"
      :totalCount.sync="tableData.totalCount" :page="param.page" @pageReset="getDictTypePage">
    </CommonPagination>

    <!-- 弹窗, 字典数据 -->
    <DictData v-if="dictDataVisible" ref="dictData"></DictData>
    <!-- 弹窗, 新增 / 修改 -->
    <AddOrUpdate v-if="addOrUpdateVisible" ref="addOrUpdate" @refreshDataList="getDictTypePage"></AddOrUpdate>
  </div>
</template>

<script>
import { getDictTypePage ,updateDictTypeStatus,deleteDictType} from "@/api/system/dict";
...
export default {
  ...
  props: {},
  // 局部注册的组件
  components: { CommonControlCard, CommonSearchReset, CommonPagination, DictData, AddOrUpdate },
  // 组件状态值
  data() {
    return {
      param: {
  ...
  methods: {
    ...
    dictDataHandle(dictType) {
      this.dictDataVisible = true
      this.$nextTick(() => {
        this.$refs.dictData.getDictDataPage(dictType)
      })
    },
    addOrUpdateHandle(dictId) {
      this.addOrUpdateVisible = true
      this.$nextTick(() => {
        this.$refs.addOrUpdate.init(dictId)
      })
    },
    getDictTypePage() {
      getDictTypePage(this.param).then(({ data: res }) => {
    ...
    updateDictTypeStatus(val){
      updateDictTypeStatus(val).then(({ data: res }) => {
        if (res.code === this.$OkCode) {
    ...
    deleteDictType(val){
      var ids = val ? [val.dictId] : this.dataListSelections.map(item => {
        return item.dictId
      })
      if(ids.length == 0){
      ...
      this.$confirm(`${val ? `确定对[${val.dictName}]进行删除操作` : `确定对[id=${ids.join(',')}]进行批量删除`}操作?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteDictType(ids).then(({ data: res }) => {
        ...
        this.$message.info(`取消${val ? `删除[${val.dictName}]` : '批量删除'}`);
      })
    },
  },
  // 计算属性
